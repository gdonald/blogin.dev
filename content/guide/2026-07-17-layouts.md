...
The first call runs the block and stores its HTML under the key; every later call
with the same key returns the stored HTML without running the block again. On a
site with a heavy sidebar and hundreds of pages this is the difference between
rendering that markup once and rendering it hundreds of times.

Only wrap content that is identical on every page. Anything that varies, the
current section, the post, a per-page value, must not go through
`cache-fragment`, or every page will show the first page's version. The cache is
