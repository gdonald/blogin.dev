...
same section. It also exposes `toc-html` and `has-toc` for a table of contents. A
listing exposes `posts` and `pagination-html`. The nav tree is available as
`nav-nodes`, with `nav-current(node)` marking the current section.
